...
my $fimagefile = $icondir."fweather.png";
...
sub copy_image {
   ...
   if ($hour > 11){
		...
		if(($hour) < int(substr($morning,0,1)) && ($minute) < int(substr($morning,2,3))){
			$imagesub = "n";
		}else{
			$imagesub = "d";
		}
	...
	File::Copy::copy($icondir.$data{'cc'}.$imagesub.".png", $imagefile) 
		...
		$imagefile;
		File::Copy::copy($icondir.$data{'fc2'}.$imagesub.".png", $fimagefile) 
			or die $icondir . $data{'fc2'}.$imagesub.".png" . " could not be copied to " . 
			$fimagefile;
}
